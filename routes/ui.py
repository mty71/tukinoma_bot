...
async def guild_dashboard(request: Request, guild_id: str):
  ...
  # 🔑 対象サーバーのデータ（リストまたは辞書）を取得
  guild_alarms = all_alarms.get(guild_id, [])
  guild_vc_settings = all_vc_settings.get(guild_id, {}) if is_admin else {}

  # もし vc_notifier が { "vc_id": { ... } } という辞書形式ならそのまま、
  # リスト形式なら { "guild_id": [...] } のようにテンプレート側が期待する形に合わせます
  server_alarms = {guild_id: guild_alarms}
  server_vc_settings = (
      {guild_id: guild_vc_settings}
      if isinstance(guild_vc_settings, list)
      else guild_vc_settings
  )
  ...
  return templates.TemplateResponse(
      ...
      context={
          ...
          "vcs": guild_vc_settings,  # 🔑 vc_section.html が 'vcs' を参照している場合用に追記
