...
async def callback(request: Request, code: str):
    ...
    if guilds_res.status_code != 200:
        return {"status": "error", "message": "Failed to fetch user guilds"}
    ...
    # ユーザーがBotと同じサーバーに入っているか判定（共通のサーバーIDを抽出）
    common_guilds = user_guild_ids.intersection(bot_guild_ids)
    ...
    # 認証成功時：Cookieにユーザー情報を保存してダッシュボードトップへ戻す
